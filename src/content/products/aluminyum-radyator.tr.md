---
lang: tr
slug: aluminyum
name: Alüminyum Radyatörler
categorySlug: dizayn-radyator
series: Alüminyum
tagline: Hafif gövde, hızlı ısı yayılımı.
description: Yatay ve dikey serilerde 11 model. Hafif alüminyum gövde, geniş renk paleti, mimari projelerde dekoratif vurgu.
featured: true
order: 1
coverKind: radiator
coverBg: terracotta-soft
heroImage: /wp-content/uploads/2021/03/lugo_yatay_beyaz-scaled-1-1024x683.jpg
heroImageAlt: LUGO Alüminyum Dizayn Radyatör
variants:
  - name: LUGO
    image: /wp-content/uploads/2021/03/lugo_yatay_beyaz-scaled-1-1024x683.jpg
    note: Yatay + Dikey · Yat 600 × 512–1336 mm · Dik 1800 × 306–512 mm · Beyaz / Siyah
  - name: CORDOBA
    image: /wp-content/uploads/2021/03/cordoba_yatay-2-1536x1024-1-1024x683.jpg
    note: Yatay + Dikey · 700 / 1200 × 500 / 600 mm · Beyaz / Siyah
  - name: MALAGA
    image: /wp-content/uploads/2021/03/malaga_yatay-scaled-1-1024x683.jpg
    note: Yatay + Dikey · Yat 600 × 850–1230 mm · Dik 1800 × 280–470 mm
  - name: SOLEY
    image: /wp-content/uploads/2021/03/soley_siyah-1024x683.jpg
    note: Yatay · 762 / 1200 × 500 / 600 mm · Siyah Mat & Parlak
  - name: ZAFRA
    image: /wp-content/uploads/2021/03/zafra-beyaz-2-scaled-1-1024x683.jpg
    note: Yatay · 724 / 1244 × 500 / 600 mm · Beyaz / Siyah
  - name: SAMARA
    image: /wp-content/uploads/2021/03/SAMARA_YATAY_BEYAZ-1536x1024-1-1024x683.jpg
    note: Yatay + Dikey · Yat 600 × 482–1346 mm · Dik 1800 × 266–482 mm · Beyaz / Siyah
  - name: ADRIANA
    image: /wp-content/uploads/2021/03/adriana_siyah-1536x1024-1-1024x683.jpg
    note: Dikey · 700 / 1200 × 500 / 600 mm · Siyah Mat & Krom Parlak
  - name: NADIA
    image: /wp-content/uploads/2021/03/nadia_siyah-1536x1024-1-1024x683.jpg
    note: Dikey · 700 / 1200 × 500 / 600 mm · Bağlantı 475 mm
  - name: TANYA
    image: /wp-content/uploads/2021/03/tanya_parlak-1536x1024-1-1024x683.jpg
    note: Dikey · 700 / 1200 × 500 / 600 mm · Parlak / Siyah · Bağlantı 475–575 mm
  - name: DINA
    image: /wp-content/uploads/2021/03/dina_vertical-2-1536x1024-1-1024x683.jpg
    note: Dikey · 1800 × 280 / 375 / 470 mm · Mimari dikey aks
  - name: DAROCA
    image: /wp-content/uploads/2021/03/daroca-1-scaled-1-1024x683.jpg
    note: Yatay · 835 / 1255 × 500 / 600 mm · Bağlantı 475–575 mm
specs:
  - label: Gövde
    value: Ekstrüzyon alüminyum
    group: Malzeme
  - label: Boya
    value: Elektrostatik toz boya (RAL paleti)
    group: Malzeme
  - label: Yükseklik aralığı
    value: 600 – 1800
    unit: mm
    group: Boyut
  - label: Genişlik aralığı
    value: 266 – 1346
    unit: mm
    group: Boyut
  - label: Çalışma basıncı
    value: '6'
    unit: bar
    group: Performans
  - label: Test basıncı
    value: '10'
    unit: bar
    group: Performans
  - label: Maks. sıcaklık
    value: '110'
    unit: °C
    group: Performans
certifications:
  - CE
  - TSE
downloads:
  - label: Alüminyum Radyatör Katalog
    kind: catalog
    url: https://media.sanicaisi.com.tr/catalogs/aluminyum-radyator.pdf
    sizeBytes: 4800000
---

Sanica Alüminyum Dizayn Radyatörler, **hafif gövde** ve **hızlı ısı yayılım** karakteri ile mimari projelerde dekoratif rol üstlenir. Yatay ve dikey serilerde 11 model — küçük antreden geniş salona kadar farklı oda büyüklüklerine uygun.

## Yönelim grupları

- **Yalnız yatay** — SOLEY, ZAFRA, DAROCA. Pencere altı, alçak duvar.
- **Yalnız dikey** — ADRIANA, NADIA, TANYA, DINA. Dar duvarda yüksek profil; mimari dikey aks.
- **Yatay + dikey** — LUGO, CORDOBA, MALAGA, SAMARA. Aynı seri içinde her iki yönelim, proje boyunca tutarlı dil.

## Boyut aralığı

Yataylar 600 / 700 / 724 / 762 / 835 / 1200 / 1244 / 1255 mm yükseklikte; 482–1346 mm genişliklerle. Dikeyler 1800 mm tek yükseklik, 266–512 mm dar profillerle. **Bağlantı mesafesi 32–575 mm** aralığında — projedeki boru hattına göre seçim.

## Renkler

Standart **Mat Beyaz** (RAL 9016) ve **Siyah** (Mat + Parlak). TANYA ve ADRIANA serilerinde krom parlak bitiş seçeneği. RAL paletinde özel renk siparişi minimum sipariş şartıyla mümkün.

## Bağlantı

Standart yan bağlantı (sol/sağ), 4 noktadan. Alttan-orta bağlantı (BCM) opsiyonel modellerde mevcut. Termostatik vana uyumlu.
