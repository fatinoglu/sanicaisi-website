---
lang: tr
slug: celik-dizayn
name: Çelik Dizayn Radyatörler
categorySlug: dizayn-radyator
series: Çelik Dizayn
tagline: 16 model · 9 renk · mimari hatlar.
description: DKP çelik gövde, geniş RAL paleti. Modern oturma odaları ve ofisler için tasarım radyatör koleksiyonu.
order: 2
coverKind: radiator
coverBg: sand
heroImage: https://www.sanicaisi.com.tr/wp-content/uploads/2021/03/vigo_yatay_beyaz-scaled-1-1024x683.jpg
heroImageAlt: VIGO Çelik Dizayn Radyatör
variants:
  - name: ALİAS
    image: https://www.sanicaisi.com.tr/wp-content/uploads/2021/03/alias-2-scaled-1-1024x683.jpg
    note: Yatay · 350 × 500 mm · Bağlantı 470 mm
  - name: ARTA
    image: https://www.sanicaisi.com.tr/wp-content/uploads/2021/03/arta-scaled-1-1024x683.jpg
    note: Dikey · 800 / 1000 / 1500 × 500 mm · Bağlantı 470 mm
  - name: CURVY
    image: https://www.sanicaisi.com.tr/wp-content/uploads/2021/03/curvy-scaled-1-1024x683.jpg
    note: Dikey · 2000 × 276 mm · Eğrisel form
  - name: ELDA
    image: https://www.sanicaisi.com.tr/wp-content/uploads/2021/03/elda-scaled-1-1024x683.jpg
    note: Dikey · 775 / 1200 × 500 mm · Bağlantı 100 mm
  - name: ESSO
    image: https://www.sanicaisi.com.tr/wp-content/uploads/2021/03/esso-scaled-1-1024x683.jpg
    note: Yatay · 1150 × 500 mm · Bağlantı 70 mm
  - name: FREDO
    image: https://www.sanicaisi.com.tr/wp-content/uploads/2021/03/fredo-scaled-1-1024x683.jpg
    note: Dikey · 1800 × 445 mm · Bağlantı 445 mm
  - name: LAZA
    image: https://www.sanicaisi.com.tr/wp-content/uploads/2021/03/laza_yatay-1536x1024-1-1024x683.jpg
    note: Yatay + Dikey · Yat 550 × 413–1180 mm · Dik 1800 × 295–531 mm · Single / Double
  - name: NİNA
    image: https://www.sanicaisi.com.tr/wp-content/uploads/2021/03/nina-alpha-scaled-1-1024x683.jpg
    note: Yatay · 350 × 500 mm · Bağlantı 470 mm
  - name: VİGO
    image: https://www.sanicaisi.com.tr/wp-content/uploads/2021/03/vigo_yatay_beyaz-scaled-1-1024x683.jpg
    note: Yatay + Dikey · Yat 600 × 588–1402 mm · Dik 1800 × 292–514 mm · Single / Double
  - name: AZRA
    image: https://www.sanicaisi.com.tr/wp-content/uploads/2021/03/azra_alpha-scaled-1-1024x683.jpg
    note: Dikey · 800 / 1200 × 500 mm · Bağlantı 60 mm
  - name: BRAGA
    image: https://www.sanicaisi.com.tr/wp-content/uploads/2021/03/braga-scaled-1-1024x683.jpg
    note: Dikey · 800 / 1200 × 500 mm · Bağlantı 470 mm
  - name: CALMO
    image: https://www.sanicaisi.com.tr/wp-content/uploads/2021/03/calmo-alpha-scaled-1-1024x683.jpg
    note: Dikey · 960 / 1300 × 500 mm · Bağlantı 468 mm
  - name: MANİLA
    image: https://www.sanicaisi.com.tr/wp-content/uploads/2021/03/manila_alpha-scaled-1-1024x683.jpg
    note: Yatay · 350 × 500 mm · Bağlantı 470 mm
  - name: MIRA
    image: https://www.sanicaisi.com.tr/wp-content/uploads/2021/03/mira-alpha-scaled-1-1024x683.jpg
    note: Yatay · 350 × 500 mm · Bağlantı 470 mm
  - name: NELAS
    image: https://www.sanicaisi.com.tr/wp-content/uploads/2021/03/nelas-scaled-1-1024x683.jpg
    note: Yatay · 350 × 500 mm · Bağlantı 470 mm
  - name: SANTONA
    image: https://www.sanicaisi.com.tr/wp-content/uploads/2021/03/santona_alpha-scaled-1-1024x683.jpg
    note: Dikey · 950 / 1300 × 500 mm · Bağlantı 440 mm
specs:
  - label: Gövde
    value: DKP çelik
    group: Malzeme
  - label: Boya
    value: Çift kat elektrostatik (9 standart renk + RAL özel)
    group: Malzeme
  - label: Yükseklik aralığı
    value: 350 – 2000
    unit: mm
    group: Boyut
  - label: Genişlik aralığı
    value: 276 – 1402
    unit: mm
    group: Boyut
  - label: Bağlantı mesafesi
    value: 60 – 470
    unit: mm
    group: Tesisat
  - label: Çalışma basıncı
    value: '6'
    unit: bar
    group: Performans
  - label: Test basıncı
    value: '10'
    unit: bar
    group: Performans
  - label: Maks. sıcaklık
    value: '110'
    unit: °C
    group: Performans
certifications:
  - CE
  - TSE
downloads:
  - label: Çelik Dizayn Radyatör Katalog
    kind: catalog
    url: https://media.sanicaisi.com.tr/catalogs/celik-dizayn-radyator.pdf
    sizeBytes: 5200000
---

Sanica Çelik Dizayn Radyatör koleksiyonu, klasik panel radyatörün ısıl performansını **mimari karaktere** taşıyan 16 farklı modelden oluşur. DKP çelik gövde, dayanıklı kaynak ve geniş renk paleti — projelerde belirleyici detay.

## Yönelim grupları

- **Yalnız yatay** — ALİAS, ESSO, NİNA, MANİLA, MIRA, NELAS. Pencere altı veya alçak duvar uygulamaları.
- **Yalnız dikey** — ARTA, CURVY, ELDA, FREDO, AZRA, BRAGA, CALMO, SANTONA. Dar duvarda yüksek profil; hol, antre, ofis.
- **Yatay + dikey** — LAZA, VİGO. Aynı seri içinde Single / Double konfigürasyon, mimari proje boyunca tek dil.

## Karakter farklılıkları

- **CURVY** — 2000 mm yükseklik, 276 mm dar profil; eğrisel hat ile dekoratif aks.
- **FREDO** — 1800 × 445 mm; sade kolon profili, geleneksel klasik tını.
- **LAZA / VİGO** — Single ve Double yüzey varyantı; yüksek ısı yükü esnekliği.
- **350 × 500 mm yatay grup** (ALİAS, NİNA, MANİLA, MIRA, NELAS) — kompakt boy; banyo, mutfak, küçük antre için.

## Renk paleti

9 standart renk: Mat Beyaz (RAL 9016), Mat Siyah, Antrasit, Krom, Mat Krom, Bakır, Bronz, Altın, Antik Sarı. **RAL paletinde özel renk** siparişi minimum sipariş şartıyla.

## Bağlantı

Yan bağlantı (sol/sağ) ve alttan-orta bağlantı (BCM) seçenekleri. Bağlantı mesafesi 60–470 mm arası; projedeki boru hattına göre seçim.
